...
```
server/
├─ bruno/                     # Copied unchanged from project root
...
├─ media/                               # Copied unchanged from project root
├─ model/
...
└─ wrapper/              # Source project files (copied unchanged)
   └─ ...
```

> **Note:** The `server/webserver/conf/ssl/` directory is intentionally empty. SSL certificates are auto-generated by wrapper.exe on first start if none exist.
...
**NGINX fails to start**: verify `php-cgi.exe` exists at `server/webserver/php/php-cgi.exe` and the `server/webserver/conf/ssl/` directory exists (even if empty). The wrapper edits `nginx.conf` at runtime; if you edited it manually, restore it from `server/webserver/conf/nginx.conf`.
